...
# IC sin CCIBARRA, y agregamos EPALMA, DSALAZAR
...
def show_main_app():
    ...
    if user_code in TL_USERS:
        if st.button("Asignar Roles"):
            ...
            # NO movemos la lógica original, se queda random.sample:
    ...
    # ------------------- Top 3 -------------------
        ...
        if st.session_state.get("show_top3_form"):
            with st.form("top3_add_form"):
                ...
                # Filtrar colaboradores según el equipo
                ...
                colaboradores = st.multiselect("Colaboradores (opcional)", 
                                               ...
                                               format_func=lambda x: valid_users.get(x, x))
            ...
            if submit_new_top3:
                ...
                final_collaborators = list(set(final_collaborators))  # quitar duplicados
    ...
    # ------------------- Action Board -------------------
        ...
        if st.session_state.get("show_action_form"):
            with st.form("action_add_form"):
                ...
                colaboradores = st.multiselect("Colaboradores (opcional)", 
                                                 options=all_collab_options,
                                                 format_func=lambda x: f"{valid_users[x]} ({x})" if x in valid_users else x)
                privado = st.checkbox("Marcar como privado")
