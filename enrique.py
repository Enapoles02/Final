...
import pandas as pd
#import time
#from streamlit_autorefresh import st_autorefresh  # Comentado para evitar refrescos automáticos
...
# ================================
# Función para mostrar el timer de 30 minutos (sin auto-refresh)
# ================================
def show_timer():
    # Comentamos st_autorefresh para evitar refresh infinito
...
# ================================
def send_action_to_top3(action_obj):
    if not hasattr(action_obj, "to_dict"):
        action_obj = action_obj.get()
    data = action_obj.to_dict()
    new_data = data.copy()
...
def show_kpi_dashboard():
    ...
    delayed_top3 = [doc for doc in db.collection("top3").stream() 
                    if datetime.strptime(doc.to_dict().get("fecha_compromiso", "2100-01-01"), "%Y-%m-%d").date() < date.today()]
    delayed_actions = [doc for doc in db.collection("actions").stream() 
                    if datetime.strptime(doc.to_dict().get("fecha_compromiso", "2100-01-01"), "%Y-%m-%d").date() < date.today()]
    st.metric("Tareas delayed (Top 3)", f"{len(delayed_top3)}")
    st.metric("Acciones delayed", f"{len(delayed_actions)}")
    ...
    if attendance_list:
        df_att = pd.DataFrame(attendance_list).reset_index(drop=True)
        st.dataframe(df_att)
    else:
...
def show_main_app():
    ...
    # Mostrar timer (sin auto-refresh para evitar problemas)
    ...
    # Menú principal (se añaden Consultorio Optimizacion y Contacto)
    ...
    # ------------------- Top 3 -------------------
        ...
        for u, t_list in groups.items():
            ...
            for task_data in t_list:
                ...
                st.markdown(f"**Status:** <span style='color:{status_colors.get(task_data.get('status',''), 'black')};'>{task_data.get('status','')}</span>", unsafe_allow_html=True)
        ...
        if st.session_state.get("show_top3_form"):
            with st.form("top3_add_form"):
                ...
                if user_code in group_ic:
                    collab_options = list(group_ic - {user_code})
                elif user_code in group_fa:
                    collab_options = list(group_fa - {user_code})
                else:
                    collab_options = list(group_namer.union(group_latam) - {user_code})
                colaboradores = st.multiselect("Colaboradores (opcional)", options=collab_options,
        ...
        for u, acts in groups_actions.items():
            ...
            for act_data in acts:
                ...
                if origen_field:
                    ...
                    st.markdown(f"**Usuario:** {valid_users.get(current_usuario, current_usuario)}")
                st.markdown(f"**Status:** <span style='color:{status_colors.get(act_data.get('status',''), 'black')};'>{act_data.get('status','')}</span>", unsafe_allow_html=True)
            ...
            if tasks_actions:
                for action in tasks_actions:
                    ...
                    if origen_field:
                        ...
                        st.markdown(f"**Usuario:** {valid_users.get(current_usuario, current_usuario)}")
                    st.markdown(f"**Status:** <span style='color:{status_colors.get(action_data.get('status',''), 'black')};'>{action_data.get('status','')}</span>", unsafe_allow_html=True)
