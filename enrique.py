...
def show_kpi_dashboard():
    ...
    # Contar tareas delayed (fecha compromiso < hoy)
...
def show_main_app():
    ...
    # Mostrar botón de Asignar Roles para TL (siempre visible para TL)
    ...
    if menu_choice == "Asistencia":
        if user_code not in TL_USERS:
            ...
            energy_level = st.radio("Seleccione:", options=energy_options, horizontal=True)
            battery_html = f"""
    ...
    # ------------------- Top 3 -------------------
        ...
        for u, t_list in groups.items():
            ...
            for task_data in t_list:
                ...
                if st.button("Editar status", key=f"edit_top3_{task_data.get('id')}"):
        ...
        if st.session_state.get("show_top3_form"):
            with st.form("top3_add_form"):
                ...
                # Filtrar colaboradores según el equipo del usuario
    ...
    # ------------------- Action Board -------------------
        ...
        for u, acts in groups_actions.items():
            ...
            for act_data in acts:
                ...
                if st.button("Editar status", key=f"edit_action_{act_data.get('id')}"):
        ...
        if user_code not in TL_USERS:
            ...
            if tasks_actions:
                for action in tasks_actions:
                    ...
                    if st.button("Editar status", key=f"edit_action_{action_data.get('id')}"):
